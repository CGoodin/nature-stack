...
int main(int argc, char *argv[]){
    ...
    const int global_fallback_max = 5;  
        ...
        if (global_path.poses.size() > 0 && odom_rcvd && grid.data.size() > 0){
            ...
            if (escape_mode_active && nav2_escape_path.poses.size() > 0) {
                ...
                if (dist_to_goal < 20.0) { 
                    ...
                    // Feed Nav2 path directly into the spline generator
            ...
            if (!path_found){
                ...
                global_fallback_counter++;
                if (global_fallback_counter > global_fallback_max){
                    global_fallback_counter = global_fallback_max;
                }
            } else {
                old_path_still_good = true;
                using_global_fallback = false;
                global_fallback_counter = 0;
            }
